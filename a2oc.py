...
def process_frame(image):
    image = np.reshape(image,[np.prod(np.shape(image))]) / 255.0
    return image
    ...
    def __init__(self,s_size,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            self.beta = slim.fully_connected(hidden, num_options,
                ...
                biases_initializer=None)
            policy = tf.nn.softmax(tf.reshape(policy, [-1, num_options, a_size]), dim=2)
            self.policy = tf.clip_by_value(policy, 1e-10, 1.0)
            ...
            if scope != 'global':
                ...
                self.value_loss = tf.reduce_mean(tf.square(self.target_v-self.q_option))
                self.entropy = tf.reduce_mean(self.policy * tf.log(self.policy))
                self.policy_loss = - tf.reduce_mean(tf.log(self.responsible_output)*self.advantages_U)
                self.beta_loss = tf.reduce_mean((self.advantages_omg + ita) * self.betas_omg)
                self.loss = 0.5 * self.value_loss + self.policy_loss + self.entropy * 0.01 + self.beta_loss
        ...
        with sess.as_default(), sess.graph.as_default():                 
            while not coord.should_stop():
                ...
                while not terminals:
                    ...
                    a = np.argmax(pi == a) 
                    s1, r_raw, terminals, _ = self.env.step(a)
                    if terminals == False:
                        s1 = process_frame(s1)
                        c = 0 
                    else:
                    ...
                    s = s1
                    q_options, betas, pi = q_options_[0], betas_[0], pi_[0]
                    #betas = betas_[0]
                    #pi = pi_[0]
                ...
                if episode_count % 200 == 0 and episode_count != 0:
                    ...
                    l = v_l + p_l + b_l
                    if episode_count % 2000 == 0 and self.name == 'worker_0' and l < 100:
...
task = 'BreakoutNoFrameskip-v4' #benchmark.tasks[1]
...
env = get_env(task)
a_size = 4#env.action_space.n
#print a_size
global_episodes = tf.Variable(0,dtype=tf.int32,name='global_episodes',trainable=False)
trainer = tf.train.AdamOptimizer(learning_rate=0.00005)
