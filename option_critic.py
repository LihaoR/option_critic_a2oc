...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  5 23:33:37 2018
    ...
    def __init__(self,s_size,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            if scope != 'global':
                ...
                self.target_v = tf.placeholder(shape=[None,num_options],dtype=tf.float32)
                ...
                self.value_loss = tf.reduce_sum(tf.square(self.target_v-self.q_options))
...
class Worker():
    ...
    def train(self,rollout,sess,gamma,returns):
        ...
        #print is_initial_betas, np.shape(is_initial_betas)
        #print 'betas_omg', betas_omg, np.shape(betas_omg)
        betas_omg = betas_omg * (1-is_initial_betas)
        #print 'betas_omg', betas_omg, np.shape(betas_omg)
        betas_OMG = np.vstack(betas_omg)
        #print betas_OMG
        betas_omg = np.zeros([batch_size])
        for i in range(len(betas_OMG)):
        ...
        #print 'advantages_U', advantages_U
        advantages_u = np.zeros([batch_size])
        for i in range(len(advantages_U)):
        ...
        v_l,p_l,_ = sess.run([self.local_OC.value_loss,
            ...
            while not coord.should_stop():
                ...
                while not terminals:
                    ...
                    if len(episode_buffer) == batch_size and terminals != True:
                        ...
                        #print returns
                        v_l,p_l = self.train(episode_buffer,sess,gamma,returns[0])
                        episode_buffer = []
                        sess.run(self.update_local_ops)
                    if terminals == True:
                        break
